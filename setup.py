...
def requirements():
    ...
    print(
        ...
        + "]"
        + cy
        + " it will take upto 10 min to install csv merge."
    )
    input_csv = input(
        gr + "[" + cy + "+" + gr + "]" + cy + " do you want to enable csv merge (y/n): "
    ).lower()
    if input_csv == "y":
        csv_lib()
    else:
...
def update_tool():
    ...
    banner()
    source = r.get(
        "https://raw.githubusercontent.com/th3unkn0n/TeleGram-Scraper/master/.image/.version"
    )
    if source.text == "3":
